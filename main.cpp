...
unsigned int nStakeMinAge = 86400;	//minimum age for coin age:  24h,60*60*24
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;	//stake age of full weight:  30d 60*60*24*30
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1496248698;
...
const string strMessageMagic = "MudanCoin Signed Message:\n";
...
static int FINAL_POW_COUNT = CUTOFF_POW_BLOCK;
int GetPowHeight(const CBlockIndex* pindex)
{
	if(NULL == pindex) return 0;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if(nHeight>1)
        nPoWHeight = GetPowHeight(pindex);
     else 
       nPoWHeight = nHeight;

     if(nPoWHeight < 11 )     	nSubsidy = MAX_MONEY / 10;
     else                               nSubsidy = 0;
...
{
    int64 nSubsidy = nCoinAge * MAX_CLOAK_PROOF_OF_STAKE / 365;
    if (nTime > 2127487098) {
    	nSubsidy = 0;
    }
...
static const int64 nTargetTimespan = 60 * 20; // 20 blocks  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MudanCoin
    bool fStrictPayToScriptHash = true; // Always active in MudanCoin
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const
{
    ...
    CBigNum bnCoinDay = bnCentSecond * CENT / COIN / 86400;//86400 = 24 * 60 * 60
    if (fDebug && GetBoolArg("-printcoinage"))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MudanCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "2017-06-01 00:38:18 互联网女皇玛丽米克尔发布2017年互联网趋势报告";
        CTransaction txNew;
...
//        if( block.hashMerkleRoot != uint256("0x698c86c2925145f5a867889658697da1bacac6f8fa0287440793eedc09db8c04") )
//			{
//				std::cout<<"\n===== hashMerkleRoot Assert ====="<<std::endl;
//				std::cout<<"hashMerkleRoot: "<<block.hashMerkleRoot.ToString()<<std::endl;
//			}
//			if (hashGenesisBlock != block.GetHash()) {
//				std::cout<<"hashGenesisBlock: "<<block.GetHash().ToString()<<std::endl;
//			}
...
//        printf("block.nNonce = %u \n", block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0x7107871aba79e4e000a24bc28848e263e0c9f24fcf69588b494f2ce7297517ef"));
...
unsigned char pchMessageStart[4] = { 0x99, 0xda, 0x05, 0xc0 };
