...
{
    resize(850, 550);
    setWindowTitle(tr("MudanCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a MudanCoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About MudanCoin"), this);
    aboutAction->setToolTip(tr("Show information about MudanCoin"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for MudanCoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("MudanCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("MudanCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to MudanCoin network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid MudanCoin address or malformed URI parameters."));
    }
...
{
	 uiInterface.ThreadSafeMessageBox(_("This operation needs your wallet passphrase to unlock the wallet."), _("MudanCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
}
